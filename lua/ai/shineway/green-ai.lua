...
diezhi_skill.getTurnUseCard=function(self)
	...
	if r == 2 then return sgs.Card_Parse("@DiezhiCard=.") end
end
sgs.ai_skill_use_func["DiezhiCard"]=function(card,use,self)
	use.card = card
end
sgs.ai_skill_playerchosen["diezhi"] = function(self, targets)
...
-- jinguo
sgs.ai_skill_use["@@jinguo"] = function(self, prompt)
	if self.player:isKongcheng() then return end
	local card = self.player:getRandomHandCard()
	self:sort(self.enemies, "hp")
	local enemy = self.enemies[1]
	return "@JinguoCard="..card:getEffectiveId().."->"..enemy:objectName()
end
