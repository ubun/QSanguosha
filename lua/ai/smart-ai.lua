...
function SmartAI:askForDiscard(reason, discard_num, optional, include_equip)    
	...
	if include_equip and self:isEquip("YitianSword") then
		table.insert(to_discard, self.player:getWeapon():getId())
	end
...
function SmartAI:getAoeValueTo(card, to , from)
	...
	if self:aoeIsEffective(card, to) then
		...
		if card:inherits("ArcheryAttack") then
			...
			if to:hasSkill("qingguo") or self:isEquip("EightDiagram", to) or to:hasSkill("baichu") then
				value = value + 10
			end	
		end	
